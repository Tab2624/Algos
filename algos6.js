...
function flatten2dArray(twoDimArr) {
  var newArr = twoDimArr.flat();
  return newArr;
}

console.log(flatten2dArray(expected1));
console.log(flatten2dArray(expected2));
console.log(flatten2dArray(expected3));
...
function secondLargest(nums) {
  ...
  for (var i = 0; i < nums.length; i++) {
    if (nums[i] > max) {
      secondLargest = max;
      max = nums[i];
    } else if (secondLargest < nums[i] && nums[i] < max) {
      secondLargest = nums[i];
    }
    else if (secondLargest == max && nums[i] < max ){
      secondLargest = nums[i]
    }
  }
